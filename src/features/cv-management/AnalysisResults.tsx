...
type DetailTab = 'overview' | 'ai' | 'cv' | 'history';
...
const CandidateAnalysisPane: React.FC<{ candidate: Candidate; tab: DetailTab }> = ({ candidate, tab }) => {
  ...
  if (tab === 'history') {
    return (
      <div className="p-6">
        <WorkspaceSection title="Lịch sử hồ sơ" icon={<CalendarDays size={17} className="text-[#007aff]" />}>
          <ol className="space-y-5 border-l border-[#d2d2d7] pl-5 text-[13px]">
            <li><p className="font-medium">Hoàn tất phân tích AI</p><p className="mt-1 text-[#6e6e73]">Điểm số và bằng chứng đã được tổng hợp cho phiên hiện tại.</p></li>
            <li><p className="font-medium">Tiếp nhận hồ sơ</p><p className="mt-1 text-[#6e6e73]">{candidate.fileName}</p></li>
          </ol>
        </WorkspaceSection>
      </div>
    );
  ...
  return (
    <div className="custom-scrollbar h-full overflow-y-auto px-5 py-5 sm:px-6">
      ...
      <WorkspaceSection title="Điểm mạnh" icon={<CheckCircle2 size={17} className="text-[#34c759]" />} tone="success">
        {strengths.length ? <ul className="space-y-2.5">{strengths.slice(0, tab === 'ai' ? 8 : 5).map((item) => <li key={item} className="flex gap-2.5 text-[13px] leading-5 text-[#3a3a3c]"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#34c759]" />{normalizeVietnameseDisplay(item)}</li>)}</ul> : <p className="text-[13px] text-[#86868b]">Chưa có nhận định nổi bật.</p>}
      </WorkspaceSection>
      <WorkspaceDivider />
      ...
      <WorkspaceSection title="Cần xác minh" icon={<TriangleAlert size={17} className="text-[#ff9f0a]" />} tone="warning">
        {weaknesses.length || candidate.softFilterWarnings?.length ? (
          <ul className="space-y-2.5">
            {[...weaknesses, ...(candidate.softFilterWarnings || [])].slice(0, tab === 'ai' ? 8 : 4).map((item) => <li key={item} className="flex gap-2.5 text-[13px] leading-5 text-[#3a3a3c]"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#ff9f0a]" />{normalizeVietnameseDisplay(item)}</li>)}
          </ul>
      ...
      <WorkspaceSection title="Bằng chứng đối chiếu" icon={<FileText size={17} className="text-[#007aff]" />}>
        <div className="divide-y divide-[#e5e5ea]">
          {details.slice(0, tab === 'ai' ? 10 : 5).map((detail, index) => (
...
const AnalysisResults: React.FC<AnalysisResultsProps> = ({
  ...
  const tab = (['overview', 'ai', 'cv', 'history'].includes(params.get('tab') || '') ? params.get('tab') : 'overview') as DetailTab;
    ...
    <section className="flex h-full min-h-0 bg-white text-[#1d1d1f]">
      <aside className={`${selectedId ? 'hidden xl:flex' : 'flex'} min-h-0 w-full shrink-0 flex-col border-r border-[#d2d2d7] bg-white xl:w-[352px]`}>
        <div className="shrink-0 border-b border-[#d2d2d7] p-3">
          ...
          <div className="mt-3 flex items-center justify-between text-[11px] text-[#6e6e73]"><span>{visible.length} ứng viên</span><button type="button" onClick={() => setParam('sort', sort === 'score' ? 'name' : 'score')} className="rounded px-1 py-0.5 hover:bg-[#f2f2f5]">Sắp xếp: {sort === 'score' ? 'Điểm phù hợp' : 'Tên'}</button></div>
        </div>
          ...
          <>
            <header className="shrink-0 border-b border-[#d2d2d7] bg-white">
              ...
              <nav className="flex gap-6 overflow-x-auto px-4 text-[13px] sm:px-6" aria-label="Chi tiết ứng viên">
                {[{ key: 'overview', label: 'Tổng quan' }, { key: 'ai', label: 'Phân tích AI' }, { key: 'cv', label: 'CV' }, { key: 'history', label: 'Lịch sử' }].map((item) => <button key={item.key} type="button" onClick={() => setParam('tab', item.key)} className={`h-11 shrink-0 border-b-2 px-1 ${tab === item.key ? 'border-[#007aff] font-medium text-[#007aff]' : 'border-transparent text-[#515154] hover:text-[#1d1d1f]'}`}>{item.label}</button>)}
              </nav>
            </header>

            <div className="min-h-0 flex-1">
              {tab === 'cv' ? <CvDocumentViewer ownerKey={documentOwner} candidate={selected} /> : tab === 'history' ? <CandidateAnalysisPane candidate={selected} tab={tab} /> : (
                <div className="grid h-full min-h-0 xl:grid-cols-[minmax(300px,44%)_minmax(0,56%)]">
                  <div className="min-h-0 border-r border-[#d2d2d7]"><CandidateAnalysisPane candidate={selected} tab={tab} /></div>
                  <div className="hidden min-h-0 xl:block"><CvDocumentViewer ownerKey={documentOwner} candidate={selected} /></div>
                </div>
              )}
            </div>
          </>

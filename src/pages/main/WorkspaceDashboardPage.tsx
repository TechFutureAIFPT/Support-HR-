...
const WorkspaceDashboardPage: React.FC<WorkspaceDashboardPageProps> = ({ userEmail, currentRun, onOpenSession }) => {
  ...
  const load = useCallback(async () => {
    ...
    // Phase 2: fetch remote data in the background
    try {
      ...
      // Remote fetch failed — keep showing local data already rendered
    } finally {
      setLoading(false);
    }

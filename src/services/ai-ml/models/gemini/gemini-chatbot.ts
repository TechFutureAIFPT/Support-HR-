...
import { generateContentWithFallback } from '@/services/ai-ml/models/gemini/gemini-core';
...
export const getChatbotAdvice = async (
  ...
  const sanitizedCandidates = successfulCandidates.map(c => ({
    id: c.id, name: c.candidateName, rank: c.analysis?.['Hạng'],
    ...
    jdFitPercent: c.analysis?.['Chi tiết']?.find(item => item['Tiêu chí'].startsWith('Phù hợp JD'))
      ...
      : 0,
    title: c.jobTitle, level: c.experienceLevel
  }));
  ...
  **OUTPUT JSON: { "responseText": "string", "candidateIds": ["id1", "id2"] }`;
  ...
  try {
    ...
    console.error("Error getting chatbot advice:", error);
